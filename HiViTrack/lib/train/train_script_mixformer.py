...
from lib.models.hivitr import build_hivitr,build_hivitr_online
# forward propagation related
from lib.train.actors import MixFormerActor,HivitrActor
# for import modules
import importlib
...
def run(settings):
    ...
    elif settings.script_name == "hivitr":
        net = build_hivitr(cfg,settings)
    elif settings.script_name == "hivitr_online":
        net = build_hivitr_online(cfg,settings)
    else:
    ...
    if settings.script_name in ["mixformer_cvt", "mixformer_vit", "mixformer_convmae"]:
        ...
        loss_weight = {'ciou': cfg.TRAIN.IOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'score': cfg.TRAIN.SCORE_WEIGHT}
        actor = MixFormerActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, run_score_head=True)
    elif settings.script_name in ["hivitr"]:#无spm
        objective = {'ciou': ciou_loss, 'l1': l1_loss}
        loss_weight = {'ciou': cfg.TRAIN.IOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT}
        actor = HivitrActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings)
    elif settings.script_name in ["hivitr_online"]:
        objective = {'ciou': ciou_loss, 'l1': l1_loss, 'score': BCEWithLogitsLoss()}
        loss_weight = {'ciou': cfg.TRAIN.IOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'score': cfg.TRAIN.SCORE_WEIGHT}
        actor = HivitrActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, run_score_head=True)
    else:
        raise ValueError("illegal script name")
