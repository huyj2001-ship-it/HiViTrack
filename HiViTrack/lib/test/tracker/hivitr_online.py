...
import cv2
import os
from lib.models.hivitr import build_hivitr_online
from lib.test.tracker.tracker_utils import Preprocessor_wo_mask
from lib.utils.box_ops import clip_box


class HivitrOnline(BaseTracker):
    def __init__(self, params, dataset_name):
        super(HivitrOnline, self).__init__(params)
        network = build_hivitr_online(params.cfg,  training=False)
        ...
        if hasattr(self.cfg, "TEST"):
            if isinstance(self.cfg.TEST.UPDATE_INTERVALS, dict) and DATASET_NAME in self.cfg.TEST.UPDATE_INTERVALS:
                self.update_intervals = self.cfg.TEST.UPDATE_INTERVALS[DATASET_NAME]
            if isinstance(self.cfg.TEST.ONLINE_SIZES, dict) and DATASET_NAME in self.cfg.TEST.ONLINE_SIZES:
    ...
    def track(self, image, info: dict = None):
        ...
        # update template
        if pred_score > 0.4 and pred_score > self.max_pred_score:
