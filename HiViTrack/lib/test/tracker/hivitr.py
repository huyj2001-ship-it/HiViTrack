...
import cv2
import os
from lib.models.hivitr import build_hivitr
from lib.test.tracker.tracker_utils import Preprocessor_wo_mask
from lib.utils.box_ops import clip_box


class Hivitr(BaseTracker):
    def __init__(self, params, dataset_name):
        super(Hivitr, self).__init__(params)
        network = build_hivitr(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            out_dict, _ = self.network.forward(self.template, self.online_template, search)
        ...
        # update template
        for idx, update_i in enumerate([200]):
