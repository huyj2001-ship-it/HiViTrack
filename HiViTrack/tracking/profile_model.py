...
#from lib.models.mixformer_convmae.mixformer import Attention
from lib.models.mixformer_cvt.mixformer import Attention
#from lib.models.hivitr.hivit import Attention
...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='mixformer_online', choices=['mixformer_convmae','mixformer_cvt','mixformer_vit', 'mixformer_online','hivitr','hivitr_online'],
...
if __name__ == "__main__":
    ...
    model_module = importlib.import_module('lib.models.hivitr')
    #model_module = importlib.import_module('lib.models.mixformer_vit')
    #model_module = importlib.import_module('lib.models.mixformer_cvt')
    #model_module = importlib.import_module('lib.models.mixformer_convmae')
    ...
    if args.script == "mixformer_vit":
        model_constructor = model_module.build_mixformer_vit
        model = model_constructor(cfg)
        # get the template and search
        template = get_data(bs, z_sz)
        search = get_data(bs, x_sz)
    ...
    if args.script == "mixformer_cvt":
        model_constructor = model_module.build_mixformer_cvt
        model = model_constructor(cfg)
        # get the template and search
        template = get_data(bs, z_sz)
        search = get_data(bs, x_sz)
        ...
        # evaluate the model properties
        evaluate(model, template, search, args.online_skip, args.display_name)
    if args.script == "mixformer_convmae":
        model_constructor = model_module.build_mixformer_convmae
        model = model_constructor(cfg)
        # get the template and search
        template = get_data(bs, z_sz)
        search = get_data(bs, x_sz)
    ...
    if args.script == "hivitr":
        ...
        # evaluate the model properties
        evaluate(model, template, search, args.online_skip, args.display_name)
    if args.script == "hivitr_online":
        model_constructor = model_module.build_hivitr_online
        model = model_constructor(cfg)
        # get the template and search
        template = get_data(bs, z_sz)
        search = get_data(bs, x_sz)
